...
Further assuming lower rank of the matrix $C$ such that
$$
rank(C) = r \leq min(m, n),
$$
lead us to two implications. First, the linear combination, $l^T Y_k,$ $i = 1,...,(m-r),$ can be modeled through the distribution of the error term $\varepsilon_k$. Second, $C$ can be expressed as $C = AB$, where A is of dimension $mxr$ and B is of dimension $rxn$. Then, the above multivariate linear model can be rewritten as 
...
The matrix $A$ of dimension $mxr$, $B$ of dimension $rxn$ and $\Sigma_{\varepsilon \varepsilon}$ of the error term are the parameters that are to be estimated. 

Presentation of reduced-rank estimation requires an important matrix result called the Householder-Young or Eckart-Young Theorem (Eckart and Young, 1936). 
